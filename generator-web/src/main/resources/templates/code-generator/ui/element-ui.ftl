<el-form :inline="true" :model="submitData" class="demo-form-inline" :rules="rules" ref="ruleForm">
    <el-card class="box-card">
        <div slot="header" class="header clearfix">
            ...
            <el-button v-if="!ischeck && !isFind" class="fr" type="primary"
                       @click="validate('ruleForm')">提交
            </el-button>
        ...
        <#if classInfo.fieldList?exists && classInfo.fieldList?size gt 0>
            <#list classInfo.fieldList as fieldItem >
                <el-form-item label="${fieldItem.fieldComment}" prop="${fieldItem.fieldName}">
                    <el-input placeholder="${fieldItem.fieldComment}"
                              v-model="submitData.${fieldItem.fieldName}"></el-input>
                </el-form-item>
            </#list>
        </#if>
    </el-card>
