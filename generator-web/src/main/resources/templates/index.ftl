        ...
        $(function () {
            ...
            function addHistory(data) {
                if (window.sessionStorage) {
                    ...
                    if (valueSession !== undefined && valueSession != null) {
                        ...
                        $("#history").prepend(
                            '<button id="his-' + tableName + '" type="button" class="btn">'
                            + tableName + '</button>');
            ...
            function donate() {
                if ($("#donate").attr("show") == "no") {
                    $("#donate").html(
                        '<img src="https://raw.githubusercontent.com/moshowgame/SpringBootCodeGenerator/master/donate.png"></img>');
            ...
            $('#btnCopy').on('click', function () {
                if (!$.isEmptyObject(genCodeArea.getValue()) && !$.isEmptyObject(navigator)
                    && !$.isEmptyObject(navigator.clipboard)) {
                ...
                $.ajax({
                           ...
                           success: function (data) {
                               ...
                               $.ajax({
                                          ...
                                          dataType: "json",
                                          success: function (data) {
                                              $.toast("#当前版本:" + data.version + " | github:"
                                                      + gitVersion);
                                          }
                                      });
                           }
...
<div class="jumbotron">
    <div class="container">
        ...
        <p class="lead">
            √基于SpringBoot2+Freemarker的<a class="lead"
                                         href="https://github.com/moshowgame/SpringBootCodeGenerator">代码生成器</a><br>
            √以解放双手为目的，减少大量重复的CRUD工作<br>
            √支持mysql/oracle/pgsql三大数据库<br>
            √用DDL-SQL语句生成JPA/JdbcTemplate/Mybatis/MybatisPlus/BeetlSQL相关代码。<br>
            如果发现有SQL语句不能识别，请<a href="https://github.com/moshowgame/SpringBootCodeGenerator/issues">留言</a>，同时欢迎大家提<a
                    href="https://github.com/moshowgame/SpringBootCodeGenerator/pulls">PR</a>和<a
                    href="#" id="donate1">赞赏</a>，谢谢！<a id="version" href="#">查看版本</a>
        </p>
        ...
        <div class="input-group mb-3">
            ...
            <input type="text" class="form-control" id="returnUtil" name="returnUtil"
                   value="ResponseBaseDTO">
            <div class="input-group-prepend">
                <span class="input-group-text">包名</span>
            </div>
            <input type="text" class="form-control" id="packageName" name="packageName"
                   value="cn.allinmd.inp">
        </div>
        <div class="input-group mb-3">
            <div class="input-group-prepend">
        ...
        <p>
            <button class="btn btn-primary btn-lg disabled" id="btnGenCode" role="button"
                    data-toggle="popover" data-content="">开始生成 »
            </button>
        ...
        <div class="row" style="margin-top: 10px;">
            <div class="btn-toolbar col-md-7" role="toolbar"
                 aria-label="Toolbar with button groups">
                <div class="input-group">
                    <div class="input-group-prepend">
                        <div class="btn btn-secondary disabled setWidth" id="btnGroupAddon">常用Util
                        </div>
                    </div>
                </div>
                <div class="btn-group" role="group" aria-label="First group">
                    <#--                    <button type="button" class="btn btn-default generator" id="beanutil">BeanUtil</button>-->
                    <button type="button" class="btn btn-default generator" id="sql">SQL(CRUD)
                    </button>
        ...
        <!-- Example row of columns -->
        <div class="row" style="margin-top: 10px;">
            <div class="btn-toolbar col-md-10" role="toolbar"
                 aria-label="Toolbar with button groups">
                <div class="input-group">
                    <div class="input-group-prepend">
                        <div class="btn btn-secondary disabled setWidth" id="btnGroupAddon">
                            MybatisPlus
                        </div>
                    </div>
                </div>
                <div class="btn-group" role="group" aria-label="First group">
                    <button type="button" class="btn btn-default generator" id="mybatisPlusEntity">
                        mybatisPlusEntity
                    </button>
                    <button type="button" class="btn btn-default generator" id="mybatisPlusMapper">
                        mybatisPlusMapper
                    </button>
                    <button type="button" class="btn btn-default generator"
                            id="mybatisPlusController">mybatisPlusController
                    </button>
                </div>
            </div>
        ...
        <div class="row" style="margin-top: 10px;">
            ...
            <div class="btn-toolbar col-md-10" role="toolbar"
                ...
                <div class="btn-group" role="group" aria-label="First group">
                    <button type="button" class="btn btn-default generator" id="mybatisMapper">
                        mybatisMapper.xml
                    </button>
                    <button type="button" class="btn btn-default generator" id="mapper">mapper
                    </button>
                    <button type="button" class="btn btn-default generator" id="annotationMapper">
                        annotationMapper
                    </button>
                    ...
                    <button type="button" class="btn btn-default generator" id="service">service
                    </button>
                    <button type="button" class="btn btn-default generator" id="service.impl">
                        serviceImpl
                    </button>
                    <button type="button" class="btn btn-default generator" id="controller">
                        controller
                    </button>
                </div>
            </div>
